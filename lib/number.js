...
*/
gammaincinv.lower = function( a, p ) {
...
* @returns {Number} function value of the inverse
*/
gammaincinv.upper = function( a, p ) {
...
function compute( a, p, q ) {
	...
	var asmall =  SMALLEST_FLOAT32,
		ck = new Array( 5 );

	if ( ( p > 1 ) || (q > 1) ) {
		return NaN;
	}
	if ( ( p <= 1e-150) || ( q <= 1e-150) ) {
		return NaN;
	}
	if ( a < asmall ) {
		return NaN;
	...
	if ( k === 0 ) {
		...
		if ( r < LARGEST_FLOAT32 ) {
			...
			warning = 'Overflow problems in one or more steps of the computation.';
			warning += 'Zero is returned';
			console.log( warning );
			return 0;
		}
	}
	...
	if ( k < 2 ) {
		...
		if ( ( t > 2e-14 ) || ( n > 99 ) ) {
			warning = 'The number of iterations in the Newton method reached the upper limit N=35';
